/* scripts/dashboard.js
   Mock-based dashboard viewer: uses encrypted videos from localStorage (no network)
   - Keeps encryption helper but reads localStorage only
   - All redirects are relative (pages/ ...)
*/
...
function bindHeaderButtons() {
  ...
  document.getElementById('logoutBtn')?.addEventListener('click', () => { localStorage.removeItem('username'); window.location.href = '../index.html'; });
  ...
  document.getElementById('adminLoginBtn')?.addEventListener('click', () => {
    ...
    if (user === 'admin' && localStorage.getItem('admin_pass') === 'admin111') window.location.href = './admin.html';
    else if (typeof adminLogin === 'function') adminLogin();
    else {
      const pass = prompt('ادخل باسورد الأدمن:');
      if (pass === 'admin111') { localStorage.setItem('username','admin'); localStorage.setItem('admin_pass','admin111'); window.location.href = './admin.html'; }
